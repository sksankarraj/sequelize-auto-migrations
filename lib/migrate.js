...
import diff_obj from "deep-diff";
...
const getPartialMigration = function(actions) {
  ...
  for (let _i in actions) {
    ...
    {transaction: transaction}
          ...
          consoleOut.push(
            `removeColumn "${action.options && action.options.field ? action.options.field : action.columnName}" from table "${action.tableName}"`,
          );
        }
        break;
...
`;
  ...
  name = name.replace(" ", "_");
  const unixString = Math.floor(Date.now() / 1000).toString();
  let filename = path.join(
    migrationsDir,
    unixString + "-" + revision + (name != "" ? `-${name}` : "") + ".js",
  );
...
const executeMigration = async function(
  queryInterface,
  filename,
  useTransaction,
  pos,
  ...
  let mig = (await import(filename)).default;
